...
class CustomGAT(nn.Module):
    def __init__(self, num_prop_size, cat_prop_size, tweet_embedding_size, user_embedding_size, user_name_embedding_size, embedding_dimension, dropout=0.3):
        ...
        # Replace RGCNConv with GATConv layers
        # First layer uses 2 heads to retain the embedding_dimension (110) overall:
        self.gat1 = GATConv(embedding_dimension, embedding_dimension // 2, heads=2, concat=True, dropout=dropout)
        # Second layer uses a single head (no concatenation) to keep the dimension:
        self.gat2 = GATConv(embedding_dimension, embedding_dimension, heads=1, concat=False, dropout=dropout)
    ...
    def forward(self, num_prop, cat_prop, tweet_emb, user_emb, user_name_emb, edge_index, edge_type=None):
...
# Instantiate the model using the GAT-based architecture
model = CustomGAT(
...
# Initialize the weights for all linear layers
